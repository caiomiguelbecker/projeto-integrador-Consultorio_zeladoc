...
class Exame_View:
    ...
    def criar_componentes(self):
        ...
        self.frm_dados = ttk.Labelframe(
            self.root,
            text=Idioma.t("exame.dados_frame"),
            padding=10
        )
        ...
        self.tbl_exames = ttk.Treeview(self.root, height=10, bootstyle=PRIMARY)
        self.tbl_exames.pack(fill=BOTH, expand=True, padx=15, pady=10)

    def configurar_treeview(self):
        self.tbl_exames["columns"] = ("id", "nome")
        self.tbl_exames.column("#0", width=0, stretch=False)
        self.tbl_exames.column("id", width=60, anchor="center")
        self.tbl_exames.column("nome", width=300)
        self.tbl_exames.heading("id", text=Idioma.t("comum.id"))
        self.tbl_exames.heading("nome", text=Idioma.t("comum.nome"))
    ...
    def configurar_eventos(self):
        ...
        self.tbl_exames.bind("<<TreeviewSelect>>", self.controller.selecionar_exame)

    def preencher_campos(self, exame):
        self.limpar_campos()
        self.txt_id.config(state="normal")
        self.txt_id.insert(0, str(exame.id))
        self.txt_id.config(state="readonly")
        self.txt_nome.insert(0, exame.nome)
    ...
    def limpar_treeview(self):
        for item in self.tbl_exames.get_children():
            self.tbl_exames.delete(item)

    def get_id_selecionado(self):
        item = self.tbl_exames.selection()[0]
        return self.tbl_exames.item(item)["values"][0]

    def confirmar_exclusao(self):
        return messagebox.askyesno(
            Idioma.t("comum.confirmacao"),
            Idioma.t("exame.confirmar_exclusao"),
            parent=self.root
        )

    def ler_dados_exame(self):
    ...
    def exibir_exames(self, exames):
        self.limpar_treeview()
        for exame in exames:
            self.tbl_exames.insert("", "end", values=(exame.id, exame.nome))
